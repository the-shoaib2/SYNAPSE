  ...
  async addBaseValues(
    ...
    if ("userAgent" in zodSchema.shape) {
      newBody.userAgent = ideInfo
        ? `${ideInfo.name}/${ideInfo.version} (Synapse/${ideInfo.extensionVersion})`
        : "Unknown/Unknown (Synapse/Unknown)";
    }
  ...
  async logToOneDestination(
    ...
    try {
      ...
      if (uriComponents.scheme === "https" || uriComponents.scheme === "http") {
        ...
        // If an API key is provided, use it, otherwise use the Synapse access token
