...
export function GraphPanel({ stage, onUpdate, onSelect, isActive }: GraphPanelProps) {
  ...
  const graphStats = useMemo(() => {
    ...
    const edgeTypes = graphData.edges.reduce((acc: Record<string, number>, edge) => {
      ...
      >
        <div className="flex flex-col items-center justify-center h-full p-2">
          <div className={`
            ...
            ${node.type === 'variable' ? 'bg-green-500' : ''}
            ${node.type === 'class' ? 'bg-purple-500' : ''}
            ${node.type === 'statement' ? 'bg-orange-500' : ''}
            ${node.type === 'block' ? 'bg-red-500' : ''}
            ${!['function', 'variable', 'class', 'statement', 'block'].includes(node.type) ? 'bg-gray-500' : ''}
          `} />
    ...
    return (
      <div className="flex items-center justify-center h-32 text-gray-500">
        <div className="text-center">
          ...
          <div className="text-sm">This stage doesn't contain graph visualization data</div>
        </div>
      </div>
    );
  }
    ...
    <div className="graph-panel h-full flex flex-col">
      ...
      {graphStats && (
        <div className="mb-4 p-3 bg-gray-50 rounded-md">
          <div className="text-sm font-medium text-gray-700 mb-2">Graph Statistics</div>
      ...
      <div className="flex-1 relative border border-gray-200 rounded-md bg-gray-50 overflow-hidden">
        ...
        >
          ...
          {/* Render nodes on top */}
          {graphData.nodes.map(node => renderNode(node))}
          
          {/* Instructions */}
          {graphData.nodes.length === 0 && (
            <div className="absolute inset-0 flex items-center justify-center text-gray-500">
              <div className="text-center">
                <div className="text-lg mb-2">Empty Graph</div>
      ...
      {(selectedNode || selectedEdge) && (
        <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
          <div className="text-sm font-medium text-blue-800 mb-2">Selection Details</div>
          {selectedNode && (
            <div className="text-xs text-blue-700">
              <div>Type: Node</div>
              <div>ID: {selectedNode}</div>
              <div>Label: {graphData.nodes.find(n => n.id === selectedNode)?.label}</div>
              <div>Node Type: {graphData.nodes.find(n => n.id === selectedNode)?.type}</div>
            </div>
          )}
          {selectedEdge && (
            <div className="text-xs text-blue-700">
              ...
              <div>Source: {graphData.edges.find(e => e.id === selectedEdge)?.source}</div>
              <div>Target: {graphData.edges.find(e => e.id === selectedEdge)?.target}</div>
              <div>Edge Type: {graphData.edges.find(e => e.id === selectedEdge)?.type}</div>
            </div>
          )}
        </div>
      )}
