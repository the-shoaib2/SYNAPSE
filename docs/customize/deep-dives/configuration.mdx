---
title: "Configuration"
description: Learn how to access and manage Synapse assistant configurations through Hub or local YAML files
...
When editing this file, you can see the available options suggested as you type, or check the reference below. When you save a config file from the IDE, Synapse will automatically refresh to take into account your changes. A config file is automatically created the first time you use Synapse, and always automatically generated with default values if it doesn't exist.
...
`config.yaml` or `config.json` can handle the vast majority of necessary configuration, so we recommend using it whenever possible. However, if you need to programmatically extend Synapse configuration, you can use a `config.ts` file, placed at `~/.synapse/config.ts` (MacOS / Linux) or `%USERPROFILE%\.synapse\config.ts` (Windows).
