...
<Info>
  ...
  provider doesn't support tools, or Synapse doesn't yet support tools with it.
...
By default, Agent will ask permission when it wants to use a tool. Click `Synapse` to allow Agent mode to proceed with the tool call or `Cancel` to reject it.
