...
4. Synapse calls the tool using built-in functionality or the MCP server that offers that particular tool
5. Synapse sends the result back to the model
...
Synapse includes several built-in tools which provide the model access to IDE functionality.
