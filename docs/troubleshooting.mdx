...
#### Prompt Logs (Synapse Console)

To view prompt logs/analytics, you can enable the Synapse Console.

1. Open VS Code settings (`cmd/ctrl` + `,`)
2. Search for the setting "Synapse: Enable Console" and enable it
3. Reload the window
4. Open the Synapse Console by using the command palette (`cmd/ctrl` + `shift` + `P`) and searching for "Synapse: Focus on Synapse Console View"

![Synapse Console](/images/images/synapse-console-d387a10c2918c117c6c253a3b5f18c22.png)

### JetBrains

Open `~/.synapse/logs/core.log` to view the logs for the Synapse plugin. The most recent logs are found at the bottom of the file.
...
We are constantly making fixes and improvements to Synapse, but the latest changes remain in a "pre-release" version for roughly a week so that we can test their stability. If you are experiencing issues, you can try the pre-release by going to the Synapse extension page in VS Code and selecting "Switch to Pre-Release" as shown below.
...
If your keyboard shortcuts are not resolving, you may have other commands that are taking precedence over the Synapse shortcuts. You can see if this is the case, and change your shortcut mappings, in the configuration of your IDE.
...
Synapse can be used in [code-server](https://coder.com/), but if you are running across an error in the logs that includes "This is likely because the editor is not running in a secure context", please see [their documentation on securely exposing code-server](https://coder.com/docs/code-server/latest/guide#expose-code-server).

### I installed Synapse, but don't see the sidebar window

By default the Synapse window is on the left side of VS Code, but it can be dragged to right side as well, which we recommend in our tutorial. In the situation where you have previously installed Synapse and moved it to the right side, it may still be there. You can reveal Synapse either by using cmd/ctrl+L or by clicking the button in the top right of VS Code to open the right sidebar.
...
If you have entered a valid API key and model, but are still getting a 404 error from OpenRouter, this may be because models that do not support function calling will return an error to Synapse when a request is sent. Example error: `HTTP 404 Not Found from https://openrouter.ai/api/v1/chat/completions`
...
This can be accomplished using the following command: `Synapse: Rebuild codebase index`.
...
<Info>
  Synapse uses system message tools as a fallback for models without native tool support, so most models should work with Agent mode automatically.
</Info>

#### Check if your model has tool support

1. Not all models support native tool/function calling, but Synapse will automatically use system message tools as a fallback
...
If Synapse's autodetection isn't working correctly, you can manually add capabilities in your `config.yaml`:
...
To see what capabilities Synapse detected for your model:
...
Most Synapse features will work normally, including autocomplete and chat. However, commands that rely on codebase indexing, such as `@codebase`, `@files`, and `@folder`, will be disabled.
...
Synapse stores it's data in the `~/.synapse` directory (%USERPROFILE%.synapse\` on Windows).
