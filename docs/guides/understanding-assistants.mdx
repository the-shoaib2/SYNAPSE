...
description: "Master the two approaches to configuring AI assistants in Synapse - choosing the right path for your development workflow"
---

Every developer has unique needs when it comes to AI assistance. Some prefer the convenience of cloud-managed configurations, while others need the control and privacy of local setups. Synapse offers both paths, and this guide will help you choose the right one for your workflow.

## The Two Paths to AI Assistance

Think of Synapse's configuration options like choosing between a managed service and self-hosting. Both get you to the same destination—powerful AI assistance in your IDE—but the journey and control level differ significantly.
...
1. Open the Synapse Chat sidebar by pressing <kbd>cmd/ctrl</kbd> + <kbd>L</kbd> (VS Code) or <kbd>cmd/ctrl</kbd> + <kbd>J</kbd> (JetBrains)
...
Hub Assistants represent the "it just works" philosophy. When you [sign in to Synapse Hub](https://auth.synapse.dev/), you gain access to a curated ecosystem of pre-configured assistants that sync seamlessly across all your development environments.
...
1. **Select Your Assistant**: Click the assistant selector in your IDE's Synapse panel
...
When you edit your `config.yaml`, Synapse provides intelligent autocomplete for all available options. Save the file, and Synapse automatically reloads your configuration—no restart required.

The first time you use Synapse, it generates a `config.yaml` with sensible defaults. From there, you can customize everything from model selection to context providers, slash commands, and more.
...
- Check YAML syntax (Synapse will show errors)
- Ensure file permissions allow reading

**Autocomplete Not Working?**
- Update to the latest Synapse version
