---
title: "Synapse CLI (cn)"
description: "Context engineering on the command line"
---

<Warning>Synapse CLI (cn) is currently in Alpha</Warning>
...
`cn` uses [`config.yaml`](./reference.mdx), the exact same configuration file as Synapse. This means that you can log in to [Synapse Hub](./hub/introduction.mdx) or use your existing local configuration.
...
`cn` supports [rules](./customize/deep-dives/rules.mdx) in the same way as the Synapse IDE extensions. You can also use the `--rule` flag to manually include a rule from the hub. For example, `cn --rule nate/spanish` will tell `cn` to use [this rule](https://hub.synapse.dev/nate/spanish) to always speak in Spanish.

### Tools

`cn` supports MCP tools, which can be configured in the [same way](./customize/deep-dives/mcp.mdx) as with the Synapse IDE extensions.
