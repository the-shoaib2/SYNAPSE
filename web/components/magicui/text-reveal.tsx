  ...
  return (
    <div ref={containerRef} className={cn("relative z-0 h-[400vh]", className)}>
      <div className="sticky top-0 mx-auto flex h-screen items-center justify-center bg-transparent px-[1rem]">
        <span className="text-foreground/20 flex flex-wrap justify-center p-8 text-4xl font-bold md:p-10 md:text-5xl lg:p-12 lg:text-6xl xl:text-7xl 2xl:text-8xl">
          {words.map((word, i) => (
            <Word
              key={i}
              index={i}
...
const Word: FC<WordProps> = ({
  ...
  return (
    <span className="relative mx-2 lg:mx-3">
      <span className="text-foreground/20 absolute">{children}</span>
      <motion.span
